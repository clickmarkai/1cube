...
export function PerformanceMonitor() {
  ...
  useEffect(() => {
    ...
    perfLogger.debug('🔄 Navigation started to:', pathname, 'at', navigationStartRef.current);

    // Track when DOM is ready
    const domReady = () => {
      const domReadyTime = performance.now();
      const navigationTime = domReadyTime - (navigationStartRef.current || 0);
      perfLogger.debug(`🏁 DOM ready for ${pathname} in ${navigationTime.toFixed(2)}ms`);
    };
    ...
    const componentReady = () => {
      ...
      perfLogger.debug(`✅ Component fully loaded for ${pathname} in ${totalTime.toFixed(2)}ms`);
